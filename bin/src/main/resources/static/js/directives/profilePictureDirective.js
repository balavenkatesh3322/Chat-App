...
(function() {
  ...
  app.directive('imageCrop', function() {
    return {
      template: '<div class="ng-image-crop ng-image-crop--{{ shape }}" ng-style="moduleStyles"><section ng-style="sectionStyles" ng-show="step==1"><input type="file" accept="image/*" class="image-crop-filepicker" id="profile-upload" style="display:none"/></section><section ng-style="sectionStyles" ng-show="step==2"><canvas class="cropping-canvas" width="{{ canvasWidth }}" height="{{ canvasHeight }}" ng-mousemove="onCanvasMouseMove($event)" ng-mousedown="onCanvasMouseDown($event)" ng-mouseup="onCanvasMouseUp($event)"></canvas><div ng-style="croppingGuideStyles" class="cropping-guide"></div><div class="zoom-handle" ng-mousemove="onHandleMouseMove($event)" ng-mousedown="onHandleMouseDown($event)" ng-mouseup="onHandleMouseUp($event)"><span>&larr; zoom &rarr;</span></div><div style="line-height: .8em;color: #9CA998;"><span style="font-size:.65em">Hold and drag over the zoom area in the arrow direction to adjust the image</span></div><button ng-click="crop()" class="upload-btns save-uplaod-btn">Save Changes</button></section><section ng-style="sectionStyles" class="section-final" ng-show="step==3"><img class="final-cropped-image" ng-src="{{ croppedDataUri }}" /></section></div>',
      replace: true,
      restrict: 'AE',
      scope: {
        width: '@',
      ...
      link: function (scope, element, attributes) {
        ...
        scope.crop = function() {
          ...
          scope.seturi({arg1: scope.croppedDataUri});
          scope.step = 3;
        };

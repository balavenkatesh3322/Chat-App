app.controller('permissionsCtrl', function($location, $window, $scope,$http, $timeout, permissionsService,$routeParams) {
        ...
        angular.element(document).ready(function() {
           ...
           // urlParams = urlParams[1].split("=");
        	urlParams = id.split("=");
            $scope.initialLoad = [];
            permissionsService.getShipAccessData(urlParams[0], urlParams[1]).then(function(response){
...
//            urlParams = urlParams[1].split("=");
        	urlParams = id.split("=");
            if (urlParams[0] === 'rankcode') {
                permissionsService.deleteRankCode(urlParams[1]).then(function(response) {
                ...
                permissionsService.deleteRoleCode(urlParams[1]).then(function(response) {
                	$scope.datastatus = false;
    				$scope.informationMessage = "Access Management deleted successfully";
    				$timeout(function () { $scope.datastatus = true; }, 3000);
                ...
                permissionsService.deleteUserCode(urlParams[1]).then(function(response) {
                	$scope.datastatus = false;
    				$scope.informationMessage = "Access Management deleted successfully";
    				$timeout(function () { $scope.datastatus = true; }, 3000);
        ...
        permissionsService.getAllModules().then(function(response) {
            
            
            
            permissionsService.getParentNodes().then(function(response) {
            ...
            if (permit === 'V') {
                ...
                for (j = 0; j < $scope.modulesList.length; j++) {
                    if ($scope.modulesList[j][0] === code) {
                        ...
                        console.log('here--permits', $scope.permit[code][0]);
                        if(!$scope.permit[code][0]) {
...
//            urlParams = urlParams[1].split("=");
            ...
            permissionsService.saveRoleMasterCompositeForm(compositeJson).then(function() {
            	$scope.datastatus = false;

app.controller('activemqDetailCtrl', function($scope, $http, $rootScope,$routeParams){
	$scope.pageSize = 25;
	$scope.fieldNames = [];
	$scope.search = [];
	...
	var getGridColumns = function(){
		$http({
			method : 'GET',
			url: "/user-grid-details/?mdlcode=MQS&gridid=G2",
	...
	$scope.saveGrids = function(){
		...
		for (var i=0; i<$scope.fieldNames.length; i++){
			...
			userGridDetails.push({"columnName": $scope.fieldNames[i].title, "mappedName":$scope.fieldNames[i].key, "isSelected": checkedVal, "moduleCode": "MQS", "detailGridId":"G2"});
		}
		$http({
	...
	var update_columns = function() {
	 ...
	 var getGridData = function getGridData(){
		 $http({
		       method: 'GET',
		       url: "/get-active-mq-detail-data/?transactionId="+$routeParams.id,
		   }).then(
		       function(response) {

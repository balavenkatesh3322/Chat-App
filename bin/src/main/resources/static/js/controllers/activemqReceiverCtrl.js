app.controller('activemqReceiverCtrl', function($scope, $rootScope,$http){
	$scope.pageSize = 25;
	$scope.fieldNames = [];
	...
	var getGridColumns = function(){
		$http({
			method : 'GET',
			url: "/user-grid-details/?mdlcode=MQR&gridid=G1",
	...
	$scope.saveGrids = function(){
		...
		for (var i=0; i<$scope.fieldNames.length; i++){
			...
			userGridDetails.push({"columnName": $scope.fieldNames[i].title, "mappedName":$scope.fieldNames[i].key, "isSelected": checkedVal, "moduleCode": "MQR", "detailGridId":"G1"});
		}
		$http({
	...
	var update_columns = function() {
	 ...
	 var getGridData = function getGridData(){
		 $http({
		       method: 'GET',
		       url: "/get-active-mq-master-receiver-data/",
		   }).then(
		       function(response) {

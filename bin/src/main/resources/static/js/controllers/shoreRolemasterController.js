app.controller('ShoreRankMasterCtrl', function($scope,$rootScope, Connectivity ,$http, $window,$location, $filter, $timeout,$compile) {
	$scope.currentPage = 1;
	$scope.hidebody = true;
...
//	$scope.pageSize = 25;
	...
	$http({
		url : "/retrieve_shore_rank_master/",
		method : 'GET',
	...
	$scope.saveShoreRankMaster = function() {
		$scope.validation();
	...
	$scope.shoreRankMasterEdit = function(ranks) {
	...
	$scope.updateShoreRankMaster = function() {
		$scope.validation();
		if ($scope.errorFlag == true) {
			form_data = {
				'id' : $scope.id,
	...
	$scope.removeShoreRankMaster = function(rank) {
...
app.controller('ShoreRankMappingCtrl', function($scope, $rootScope, Connectivity,$http, $window,$location, $filter, $timeout,$compile) {
	$scope.currentPage = 1;
	$scope.hidebody = true;
	$scope.haspermission = false;
	$scope.geterrorstatuscode ="0";
...
//	$scope.pageSize = 25;
	...
	$scope.setValue = function(arg1, arg2) {
	 ...
	 Connectivity.IsOk().then(function(response){
	$http({
		url : "/retrieve_shore_rankmapping_details/",
		method : 'GET',
	...
	$scope.saveShoreRankDetail = function() {
		$scope.validation();
		if ($scope.errorFlag == true) {
			form_data = {
				'rankcode' : $scope.rankcode,
			...
			$http({
				url : "/save_shore_rankmapping_details/",
				dataType : 'json',
				method : 'POST',
				data : form_data,
				headers : {
	...
	$scope.updateShoreRank = function() {
		$scope.validation();
		if ($scope.errorFlag == true) {
			form_data = {
				'id' : $scope.id,
			...
			$http({
				url : "/update_shore_rankmapping_details/",
				dataType : 'json',
				method : 'POST',
				data : form_data,
				headers : {
	...
	$scope.removeShoreRankDetail = function(rank) {
		...
		if (result) {
			...
			$scope.rankname = "";
			$http({
				url : "/delete_shore_rankmapping_details/",
				dataType : 'json',
				method : 'POST',
				data : form_data,
				headers : {
	...
	$http({
		method : 'GET',
		url : "/retrieve_shore_departmentcode/",
